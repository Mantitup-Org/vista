        ...
        task: (ctx, task) =>
          task.newListr(
            () => [
              ...
              {
                title: 'Set Vista version in package.json',
                task: async () => {
              ...
              },
              {
                ...
                task: async () => {
                  const deploymentUrl = await deployProject(
                    ORIGIN_PROJECT_NAME,
                    originAppFolder
                  )
                  ctx.originUrl = deploymentUrl
                },
              },
            ],
            { concurrent: false }
        ...
        task: (ctx, task) =>
          task.newListr(
            () => [
              ...
              {
                title: 'pack local Vista version',
                task: async () => {
...
export async function deployProject(projectName, appFolder) {
  try {
    ...
    console.log('\x1b[31mDeployment failed:\x1b[39m', err)
    throw err
  }
}
