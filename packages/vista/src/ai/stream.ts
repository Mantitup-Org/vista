...
export class AgentStream implements AsyncIterable<StreamChunk> {
  private source: AsyncIterable<StreamChunk>;
   ...
   */
  async collect(): Promise<{ text: string; toolCalls: ToolCall[]; usage?: TokenUsage }> {
    let text = '';
...
export function createAIStreamResponse(
  stream: AsyncIterable<StreamChunk> | AgentStream,
  init?: ResponseInit
): Response {
