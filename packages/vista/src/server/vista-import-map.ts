...
function resolveSourceCandidates(packageRoot: string, subpath: string): string[] {
  ...
  const sourceMap: Record<string, string[]> = {
    link: [path.join(packageRoot, 'client', 'link.react-server'), path.join(packageRoot, 'client', 'link')],
    image: [path.join(packageRoot, 'image', 'react-server'), path.join(packageRoot, 'image', 'index')],
    router: [path.join(packageRoot, 'client', 'router.react-server'), path.join(packageRoot, 'client', 'router')],
    navigation: [
      path.join(packageRoot, 'client', 'navigation.react-server'),
      path.join(packageRoot, 'client', 'navigation'),
    ],
    dynamic: [path.join(packageRoot, 'client', 'dynamic.react-server'), path.join(packageRoot, 'client', 'dynamic')],
    script: [path.join(packageRoot, 'client', 'script.react-server'), path.join(packageRoot, 'client', 'script')],
    ...
    'client/rsc-router': [
      path.join(packageRoot, 'client', 'rsc-router.react-server'),
      path.join(packageRoot, 'client', 'rsc-router'),
    ],
