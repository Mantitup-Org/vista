import { getCurrentSegmentConfig } from './request-context';
import type { SegmentFetchCache } from './segment-config';
...
const fetchResponseCache = new Map<string, CachedFetchEntry>();

let installed = false;

function createCacheKey(input: RequestInfo | URL, init: RequestInit | undefined): string | null {
...
function cloneCachedResponse(entry: CachedFetchEntry): Response {
...
function shouldCacheResponse(policy: SegmentFetchCache, init: RequestInit | undefined): boolean {
      ...
      const url =
        typeof input === 'string'
          ...
          : input instanceof URL
            ...
            : input.url;
      throw new Error(
        `cache: '${nextInit.cache}' used on fetch for ${url} with export const fetchCache = 'only-no-store'`
      );
    }
    nextInit.cache = 'no-store';
    return nextInit;
      ...
      const url =
        typeof input === 'string'
          ...
          : input instanceof URL
            ...
            : input.url;
      throw new Error(
        `cache: 'no-store' used on fetch for ${url} with export const fetchCache = 'only-cache'`
      );
    }
    if (!nextInit.cache) {
      nextInit.cache = 'force-cache';
...
export function installSegmentFetchPolicyShim(): void {
  ...
  globalThis.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    const policy = getCurrentSegmentConfig()?.fetchCache ?? 'auto';
    ...
    const response = await originalFetch(input as any, normalizedInit as any);
    if (cacheKey) {
...
export function clearSegmentFetchCache(): void {
