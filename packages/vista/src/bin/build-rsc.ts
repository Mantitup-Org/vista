...
import { generateClientManifestWithRoots, discoverProjectClientRoots } from '../build/rsc/client-manifest';
...
function collectFrameworkClientReferences(cwd: string): string[] {
  ...
  for (const packageRoot of roots) {
    collectUseClientFiles(path.join(packageRoot, 'dist'), collected);
    collectUseClientFiles(path.join(packageRoot, 'src'), collected);
  }
...
${isDev ? getDevToolsIndicatorBootstrapSource(devToolsBootId) : ''}
${isDev ? getDevErrorOverlayBootstrapSource() : ''}
...
function waitForInlineFlightPayload(timeoutMs: number): Promise<string | null> {
  var existing = (window as any).${RSC_DATA_FLAG};
  ...
  return new Promise(function (resolve) {
    ...
    var timer = setInterval(function () {
      var value = (window as any).${RSC_DATA_FLAG};
...
waitForInlineFlightPayload(4000).then(function (inlineFlight) {
  if (typeof inlineFlight !== 'string' || inlineFlight.length === 0) {
    reportDevRuntimeError(
      'Hydration Error',
      new Error('Missing inline Flight payload (window.${RSC_DATA_FLAG}). First load no longer refetches /rsc.')
    );
    return;
  }
...
export async function buildRSC(watch: boolean = false): Promise<{
  ...
  const projectClientRoots = discoverProjectClientRoots(cwd);
  const additionalClientRoots = projectClientRoots.filter(
  ...
  if (fs.existsSync(appDir)) {
    ...
    for (const root of additionalClientRoots) {
      ...
      clientReferenceFiles.push(
        ...extraScan.clientComponents.map((component) => component.absolutePath)
      );
      if (_debug && extraScan.clientComponents.length > 0) {
        console.log(
          `[Vista JS RSC] Found ${extraScan.clientComponents.length} client components ('use client') in ${root.prefix}`
        );
      }
    }
