...
function addStandalonePathVariants(specifier: string, variants: Set<string>): void {
  ...
  if (!specifier.includes('/.vista/standalone/') && specifier.startsWith('file://')) {
    const inserted = specifier.replace(
      /^(file:\/\/\/(?:[A-Za-z]:)?(?:\/[^/]+)*)(\/(?:app|components|utils|lib|src|content|hooks)\/)/i,
      `$1${standaloneProject}$2`
    );
...
function buildSpecifierVariants(specifier: string): string[] {
  ...
  if (baseSpecifier.startsWith('file://')) {
    ...
    try {
      addStandalonePathVariants(pathToFileURL(fileURLToPath(baseSpecifier)).toString(), variants);
    } catch {
...
function splitManifestKey(key: string): { specifier: string; exportName: string | null } {
...
function normalizeManifestSpecifier(specifier: string): string {
  return specifier
    .replace(/\\/g, '/')
    .replace(/^file:\/\//i, '')
    .replace(/^\/+/, '')
...
function relativeClientPath(normalized: string): string {
  const markers = [
    '/app/',
    '/components/',
    '/utils/',
...
function exportNamesCompatible(requested: string | null, existing: string | null): boolean {
  if (requested === null || existing === null) return true;
 ...
 */
export function resolveReactClientManifestEntry(
  manifest: ReactClientReferenceManifest,
  key: string
): ReactClientReferenceManifestEntry | undefined {
  ...
  let fuzzy: ReactClientReferenceManifestEntry | undefined;
 ...
 */
export function createGuardedReactClientManifest(
  manifest: ReactClientReferenceManifest
): ReactClientReferenceManifest {
  return new Proxy(manifest, {
    get(target, prop, receiver) {
      if (typeof prop === 'string' && (prop.startsWith('file:') || prop.includes('#'))) {
        ...
        throw new Error(
          `Could not find the module "${prop}" in the React Client Manifest. ` +
