...
export interface VistaDirs {
  ...
  types: string; // .vista/types/ — Next.js .next/types equivalent
}
...
function quoteRouteLiteral(route: string): string {
  return `'${route.replace(/\\/g, '\\\\').replace(/'/g, "\\'")}'`;
}

export function writeVistaTypes(vistaDir: string, routes: string[] = []): void {
  const typesDir = path.join(vistaDir, 'types');
  ...
  fs.writeFileSync(path.join(typesDir, 'routes.d.ts'), source);
  fs.writeFileSync(
...
export function writeVistaTrace(
  vistaDir: string,
  fileTrace: { schemaVersion?: number; copiedFiles?: unknown[]; generatedAt?: string } = {}
): void {
  const payload = {
    schemaVersion: 1,
  ...
  fs.writeFileSync(path.join(vistaDir, 'trace'), JSON.stringify(payload, null, 2));
}
