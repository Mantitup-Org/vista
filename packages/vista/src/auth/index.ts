...
import { isSafeRedirectLocation } from '../server/middleware-security';
import {
  type AuthConfig,
  type AuthProvider,
  type AuthSession,
...
function requestToUrl(request: Request): URL {
  return new URL(request.url);
}

function json(data: unknown, init: ResponseInit = {}, extraCookies: string[] = []): Response {
...
function findProvider(config: ResolvedAuthConfig, id: string): AuthProvider | undefined {
...
async function handleAuthRequest(
  ...
  if (action === 'csrf' && method === 'GET') {
    const token = cookies.get(CSRF_COOKIE) || randomToken(24);
    return json({ csrfToken: token }, {}, [cookie(CSRF_COOKIE, token, { httpOnly: false, maxAge: 60 * 60 })]);
  }

  if (action === 'providers' && method === 'GET') {
    const providers = Object.fromEntries(
      config.providers.map((provider) => [provider.id, { id: provider.id, name: provider.name, type: provider.type }])
    );
  ...
  if (action === 'signin' && method === 'GET' && !providerId) {
    ...
    const buttons = config.providers
      ...
      .join('');
    return new Response(
      `<!doctype html><html><body><h1>Sign in</h1>${buttons}</body></html>`,
...
export function VistaAuth(config: AuthConfig) {
  const resolved = resolveAuthConfig(config);

  const handlers = {
    GET: (request: Request) => handleAuthRequest(request, resolved),
    POST: (request: Request) => handleAuthRequest(request, resolved),
  };

  async function auth(request?: Request): Promise<AuthSession | null> {
    if (request) {
  ...
  async function signOut(): Promise<Response> {
    ...
    return handleAuthRequest(
      new Request(`${protocol}://${host}${resolved.basePath}/signout`, { method: 'POST' }),
      resolved
    );
  }
  ...
  function authMiddleware(
    ...
    return async ({ request, next }: { request: any; next: () => Promise<Response> }) => {
