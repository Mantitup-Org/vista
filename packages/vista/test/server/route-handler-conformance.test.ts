...
test('catch-all routes [...slug] and optional catch-all [[...slug]]', async () => {
  ...
  try {
    ...
    await withRouteServer(cwd, async (origin) => {
      ...
      assert.deepEqual(await docsRootRes.json(), { slug: [] });
...
test('src/app structure resolution works seamlessly', async () => {
  const cwd = makeTempProject();
  try {
    const routeFile = path.join(cwd, 'src', 'app', 'api', 'src-test', 'route.ts');
    writeFile(
      routeFile,
      [
        'exports.GET = async function GET() {',
        '  return Response.json({ fromSrcApp: true });',
        '};',
      ].join('\n')
    );

    await withRouteServer(cwd, async (origin) => {
      const res = await fetch(origin + '/api/src-test');
      assert.equal(res.status, 200);
      assert.deepEqual(await res.json(), { fromSrcApp: true });
    });
  } finally {
    fs.rmSync(cwd, { recursive: true, force: true });
  }
