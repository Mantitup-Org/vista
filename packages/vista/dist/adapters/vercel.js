...
exports.vercelAdapter = {
    ...
    build(context) {
        ...
        // It expects vista's server bundle to be at dist/server/engine relative to the function.
...
function getHandler() {
  if (!_handler) {
    // Vista's server engine exports a request handler compatible with Node http.
    const { createRequestHandler } = require('../../../.vista/server/engine');
    _handler = createRequestHandler({ cwd: process.cwd() });
  }
  return _handler;
}
