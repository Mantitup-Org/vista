...
async function resolveProvider(modelString) {
    const [providerId, ...modelIdParts] = modelString.split(':');
    const modelId = modelIdParts.join(':');
    if (!providerId || !modelId) {
        throw new Error(`[vista/ai] Invalid model format: "${modelString}". Expected format "provider:model" (e.g. "openai:gpt-4o").`);
    }
    try {
        ...
        if (error.code === 'ERR_MODULE_NOT_FOUND' || error.message.includes('Cannot find module')) {
            ...
            throw new Error(`[vista/ai] Failed to load provider "${providerId}". Please ensure it is installed:\n  npm install ${packageName}\n\nOriginal error: ${error.message}`);
        }
        throw error;
    }
}
